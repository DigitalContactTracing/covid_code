...
def inizialize_infected_time0(self):
    for i in self.Y_i_nodes:
        ...
        self.I[i] = {'tau': tau, 'tau_p': None, 'to': onset_time(symptomatics=self.sympt, testing=self.test), 'inf': [],
                     'e_inf': [], 'ss_inf': [], 'ss_p': None, 'e_p': None}
        ...
        for m in neigh:
            if m not in self.I and m not in self.quarantined:
                ...
                if rr < pp:  # avviene il contagio di m
                    ...
                    self.I[m] = {'tau': 0, 'tau_p': self.I[node]['tau'], 'to': current_time + to, 'inf': [],
                                 'e_inf': [], 'ss_inf': [], 'ss_p': ss, 'e_p': e}
